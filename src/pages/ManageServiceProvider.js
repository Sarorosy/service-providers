...
const ManageServiceProvider = () => {
    ...
    const fetchServiceProviders = async () => {
        ...
        if (status === 'active' || filterActive) {
            url = 'https://serviceprovidersback.onrender.comapi/users/activeserviceproviders';
        } else if (status === 'inactive' || filterInactive) {
            url = 'https://serviceprovidersback.onrender.comapi/users/inactiveserviceproviders';
        } else {
            url = 'https://serviceprovidersback.onrender.comapi/users/serviceproviders'; // Default URL for all service providers
        }
    ...
    const columns = [
        {
            ...
            render: (data) => {
                const imageUrl = (data && data.trim() !== "")
                    ? `https://serviceprovidersback.onrender.comuploads/profileimg/${data}`
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this service provider?')) {
            try {
                const response = await fetch(`https://serviceprovidersback.onrender.comapi/users/${id}`, {
                    method: 'DELETE',
                });
                if (!response.ok) {
    ...
    const handleToggleStatus = async (id, currentStatus) => {
        try {
            const response = await fetch(`https://serviceprovidersback.onrender.comapi/users/${id}/status`, {
                method: 'PATCH',
            });
