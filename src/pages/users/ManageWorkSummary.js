...
const ManageWorkSummary = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(`https://elementk.in/spbackend/api/worksummaries/user/${userId}`);
      if (!response.ok) {
  ...
  const handleDelete = async () => {
    if (selectedWorkSummary) {
      try {
        const response = await fetch(`https://elementk.in/spbackend/api/worksummaries/${selectedWorkSummary._id}`, {
          method: 'DELETE',
        });
        if (!response.ok) {
  ...
  const toggleStatus = async (id, currentStatus) => {
    try {
      const response = await fetch(`https://elementk.in/spbackend/api/worksummaries/${id}/status`, {
        method: 'PATCH',
      });
