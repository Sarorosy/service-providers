...
const ManageWorkSummary = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(`https://service-providers-panel.vercel.app/api/worksummaries/user/${userId}`);
      if (!response.ok) {
  ...
  const handleDelete = async () => {
    if (selectedWorkSummary) {
      try {
        const response = await fetch(`https://service-providers-panel.vercel.app/api/worksummaries/${selectedWorkSummary._id}`, {
          method: 'DELETE',
        });
        if (!response.ok) {
  ...
  const toggleStatus = async (id, currentStatus) => {
    try {
      const response = await fetch(`https://service-providers-panel.vercel.app/api/worksummaries/${id}/status`, {
        method: 'PATCH',
      });
