...
const ManageProjects = () => {
  ...
  const handleDelete = async () => {
    if (selectedProject) {
      try {
        const response = await fetch(`https://elementk.in/spbackend/api/projects/${selectedProject._id}`, {
          method: 'DELETE',
        });
  ...
  const toggleStatus = async (id, currentStatus) => {
    try {
      const response = await fetch(`https://elementk.in/spbackend/api/projects/${id}/status`, {
        method: 'PATCH',
      });
