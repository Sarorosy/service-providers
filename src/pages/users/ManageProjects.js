...
const ManageProjects = () => {
  ...
  const handleDelete = async () => {
    if (selectedProject) {
      try {
        const response = await fetch(`https://serviceprovidersback.onrender.comapi/projects/${selectedProject._id}`, {
          method: 'DELETE',
        });
  ...
  const toggleStatus = async (id, currentStatus) => {
    try {
      const response = await fetch(`https://serviceprovidersback.onrender.comapi/projects/${id}/status`, {
        method: 'PATCH',
      });
