...
const UserWorkDays = ({ serviceProviderId, onClose }) => {
  ...
  const fetchLoginHistories = async () => {
    try {
      setLoading(true);
      const response = await fetch(`https://elementk.in/spbackend/api/login-history/${serviceProviderId}`); // Fetch login histories for the user
      if (!response.ok) {
  ...
  const fetchWorkoffs = async () => {
    try {
      const response = await fetch(`https://elementk.in/spbackend/api/workoffs/user/${serviceProviderId}`);
      if (!response.ok) {
